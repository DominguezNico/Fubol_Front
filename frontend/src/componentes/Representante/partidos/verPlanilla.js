...
function  VerPlanilla (props) {
  ...
  const getPartidosPendientes= async() => {
    await fetch(`https://futbolito-back.herokuapp.com/getPartidosPendientesClub?idClub=${props.location.state.club.idClub}`)
  ...
  const validar =   (idPart) => {
      ...
      fetch(`https://futbolito-back.herokuapp.com/validarPartido?idPartido=${idPart}&idRepresentante=${props.location.state.legajo}`, requestOptions )
      .then( () => {
