...
function ModificarDatosPersonales (props){
  ...
  const cargarAlPrincipio = async () =>{
    await fetch(`https://futbolito-back.herokuapp.com/getJugador?idJugador=${props.location.state.id}`)
  ...
  const cambios = async () => {
    ...
    if(!(tel==='')){
      setMostarTel(tel)
      await fetch(`https://futbolito-back.herokuapp.com/cambiarTelefonoJugador?id=${datosPersonales.id}&telefono=${tel}`,requestOptions)
    ...
    if(!(direc==='')){
      ...
      await fetch(`https://futbolito-back.herokuapp.com/cambiarDireccionJugador?idJugador=${datosPersonales.id}&direc=${direc}`,requestOptions)
      .then(()=>{setPendiente(false)}
      )
    }
    if(!(mail==='')){
      setMostarMail(mail)
      await fetch(`https://futbolito-back.herokuapp.com/cambiarMailJugador?id=${datosPersonales.id} &mail=${mail}`,requestOptions)
      .then(()=>{setPendiente(false)}
      )
    }
